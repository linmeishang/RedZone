...
def get_df_parquet(folder):
    ...
    df.to_parquet("N:\\agpo\\work2\\MindStep\\WP_4\\WP4_Task_4_5\\SurrogateABM\\DataCollectionRed\\"+folder[len(folder)-9:len(folder)-1]+".parquet.gzip",  compression="gzip")
...
all_folders = glob.glob(os.path.join(path + '/202*/'))
print(all_folders)

#%%
# python finds the latest .parquet.gzip
all_parquets = glob.glob(os.path.join(path+"\\total_df_RedZone_202*.parquet.gzip"))
...
total_df.to_parquet("N:\\agpo\\work2\\MindStep\\WP_4\\WP4_Task_4_5\\SurrogateABM\\DataCollectionRed\\total_df_RedZone_"+Date+".parquet.gzip",  compression="gzip")
...
#%%
#total_df.to_excel('test_1k.xlsx')
